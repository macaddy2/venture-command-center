// ============================================================
// Weekly Digest — Auto-generated portfolio summary
// ============================================================
...
export default function WeeklyDigest() {
    ...
    const weekLabel = `${weekAgo.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })} — ${today.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}`;
    ...
    const digestText = useMemo(() => {
        let text = `📊 VENTURE COMMAND CENTER — WEEKLY DIGEST\n`;
        text += `${weekLabel}\n\n`;
        text += `═══════════════════════════════════════\n\n`;
        text += `📈 PORTFOLIO OVERVIEW\n`;
        text += `• ${portfolioStats.totalVentures} ventures tracked\n`;
        text += `• ${completedThisWeek} tasks completed this week\n`;
        text += `• ${newTasksThisWeek} new tasks created\n`;
        text += `• ${blockedNow} tasks currently blocked\n`;
        text += `• Avg health score: ${portfolioStats.avgHealth}%\n\n`;

        text += `═══════════════════════════════════════\n\n`;
        text += `🏢 VENTURE UPDATES\n\n`;

        ventureSummaries.forEach(v => {
            const emoji = v.healthScore >= 70 ? '🟢' : v.healthScore >= 40 ? '🟡' : '🔴';
            text += `${emoji} ${v.name} (${v.prefix}) — Health: ${v.healthScore}%\n`;
            text += `   ${v.tier} · ${v.geo} · ${v.stage}\n`;
            ...
            if (v.upcomingMilestones.length > 0) {
                ...
                text += `   📌 Next milestone: ${ms.name} (${new Date(ms.target_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })})\n`;
            }
            text += `\n`;
        });
